      ...
      (let ((start (point)))
        ...
        (let ((exec-path (cons "/home/csr21/goldsmiths/research/src/git/audioDB" exec-path)))
          (let ((adb-process
            ...
            ;; poor-me's lexical binding
            (process-put adb-process 'adb-db db)
            (process-put adb-process 'adb-qid qid)
            (set-process-filter adb-process 'adb-query-process-filter)))
        (add-text-properties start (point-max) `(adb-db ,db adb-qid ,qid)))
      (read-only-mode +1))))

(defun adb-query-process-filter (process string)
  (let ((db (process-get process 'adb-db))
        (qid (process-get process 'adb-qid)))
    (with-current-buffer (process-buffer process)
      (goto-char (point-max))
      (read-only-mode -1)
      (insert (propertize string 'adb-db db 'adb-qid qid))
      (read-only-mode +1))))
...
(defun adb-query-play-match ()
  ...
  (let ((line (thing-at-point 'line))
        (word (thing-at-point 'word)))
    (when (= (length word) 32)
      (let ((qid (get-text-property (point) 'adb-qid))
